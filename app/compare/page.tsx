...
export default function ModelsPage() {
  ...
  const toggleModel = (name: string) => {
    ...
    if (targetModel && !targetModel.selected) {
      const currentSelectedCount = models.filter((m) => m.selected).length;
      if (currentSelectedCount >= 4) {
        alert("최대 4개의 모델까지만 선택할 수 있습니다.");
        return;
      }
    }
  ...
  const resetFilters = () => {
    setSearch("");
    setCategories(["Chatbot", "Image", "Writing", "Video"]);
    setPrices(["Free", "Paid", "Freemium"]);
  };
